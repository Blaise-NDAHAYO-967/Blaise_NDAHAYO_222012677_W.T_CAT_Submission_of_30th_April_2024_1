...
<section>
    ...
    <form method="post" action="enrollments.php">
        <label for="SubmissionID">EnrollmentID :</label>
        <input type="number" id="EnrollmentID" name="EnrollmentID" required><br><br>

        <label for="AssignmentID">student_id:</label>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
        ...
        $stmt = $connection->prepare("INSERT INTO Enrollments (EnrollmentID, student_id,CourseID,EnrollmentDate) VALUES (?, ?, ?,?)");
        $stmt->bind_param("isss", $EID, $sid, $cID,$EDate);
    ...
    <table>
        ...
        if ($result->num_rows > 0) { 
            ...
            while ($row = $result->fetch_assoc()) {
                // Store the EnrollmentID in a variable
                ...
                echo "<tr>
                    ...
                    <td><a href='delete_enrollments.php?EnrollmentID=$EID'>Delete</a></td> 
                    <td><a href='update_enrollments.php?EnrollmentID=$EID'>Update</a></td> 
                </tr>";
            }
        } else {
